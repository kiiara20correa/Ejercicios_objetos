...
class CuentaCorriente(CuentaBancaria):
    ...
    def depositar(self, monto):
        if monto > 0:
            self._saldo += monto
            print(f"Se ha depositado {monto} en la cuenta corriente de {self._nombre_titular}, saldo actual: {self.obtener_saldo()}")
        else:
    ...
    def extraer(self, monto):
        if monto <= self.obtener_saldo() and monto <= self._limite_extraccion:
            self._saldo -= monto
            print(f"Se ha extraído {monto} de la cuenta corriente de {self._nombre_titular}, saldo actual: {self.obtener_saldo()}")
        else:
            if monto > self._limite_extraccion:
                print("Usted no puede extraer ese monto (supera el límite)")
            else:
